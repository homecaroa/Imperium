  ...
  renderMilitary(state) {
    ...
    container.innerHTML = `
      <div class="rpanel-section">
        <div class="rpanel-title">⚔️ Composición del Ejército</div>
        ${(state.armyUnits || []).map(u => {
          const def = MILITARY_UNITS[u.typeId];
          if (!def) return '';
          return `<div class="mil-unit-row"><span>${def.icon} ${def.name}</span><span style="color:var(--gold2)">${u.count.toLocaleString()}</span></div>`;
        }).join('') || '<div style="color:var(--text3);font-family:var(--font-mono);font-size:10px">Sin unidades</div>'}
        ${state.legendaryUnit ? `<div class="mil-unit-row" style="color:var(--gold3)"><span>⭐ ${state.legendaryUnit.icon} ${state.legendaryUnit.name}</span><span>LEGENDARIA</span></div>` : ''}
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">
          <div class="mil-unit-row"><span>🗡️ Total</span><span>${total.toLocaleString()}</span></div>
  ...
  renderClimate(state) {
    ...
    container.innerHTML = `
      ...
      <div class="rpanel-section">
        ...
        ${[['🌾 Alimentos', cs.totalFoodMod+'%'], ['❤️ Moral', cs.totalMoralMod], ['⚔️ Ejército', cs.totalArmyMod+'%']].map(([l,v])=>`
          <div class="stat-row">
            <span class="stat-row-label">${l}</span>
            <span style="font-family:var(--font-mono);font-size:11px;color:"+(v>0?'var(--green2)':'var(--red2)')+"">"+(v>0?'+':'')+v+"</span>\n          </div>`).join('')}
      </div>
      <div class="rpanel-section">
