...
const AlthoriаMap = {
  ...
  // ── ASIGNACIÓN ALEATORIA DE ZONAS ─────────────────────────
  // Con lógica geográfica: cada nación prefiere ciertos biomas
  assignZones(gameState) {
    if (!gameState) return;
    const civ   = gameState.civId;
    const nats  = (gameState.diplomacy || []).map((n, i) => ({ id: `ai_${i+1}`, civId: n.id || 'mongol', personality: n.personality }));
    const all   = [{ id: 'player', civId: civ }, ...nats];

    // Barajar regiones con seed de la partida
    const seed  = gameState.mapSeed || 42;
    const rng   = this._seededRng(seed + 9001);
    const shuffled = [...ALTHORIA_REGIONS].sort(() => rng() - 0.5);

    // Distribuir regiones ponderando afinidad geográfica
    this.nationZones = {};
    all.forEach(n => { this.nationZones[n.id] = []; });

    // Primera ronda: dar 3-4 regiones a cada nación preferidas
    const regionsCopy = [...shuffled];
    const perNation   = Math.floor(regionsCopy.length / all.length);

    all.forEach(nation => {
      const preferred = regionsCopy.filter(r => {
        const affin = GEO_AFFINITY[r.geoType] || [];
        return affin.includes(nation.civId);
      });
      const nonPref = regionsCopy.filter(r => {
        const affin = GEO_AFFINITY[r.geoType] || [];
        return !affin.includes(nation.civId);
      });

      // Tomar 1-2 preferidas + resto aleatorio
      const take = perNation;
      let taken = 0;
      for (const r of preferred) {
        if (taken >= Math.ceil(take * 0.6)) break;
        const idx = regionsCopy.indexOf(r);
        if (idx > -1) { regionsCopy.splice(idx, 1); this.nationZones[nation.id].push(r.id); taken++; }
      }
      for (const r of nonPref) {
        if (taken >= take) break;
        const idx = regionsCopy.indexOf(r);
        if (idx > -1) { regionsCopy.splice(idx, 1); this.nationZones[nation.id].push(r.id); taken++; }
      }
    ...
    // Repartir sobrantes aleatoriamente
    regionsCopy.forEach((r, i) => {
      const nat = all[i % all.length];
      this.nationZones[nat.id].push(r.id);
    });

    // Actualizar leyenda
    this._buildLegend(gameState);

    // Generar puntos de influencia
  ...
  // ── CAPA: ICONOS DE CAPITAL ───────────────────────────────
  _renderCapitalIcons(ctx, W, H) {
    Object.entries(this.nationZones).forEach(([natId, zones]) => {
      if (!zones.length) return;
      const col    = this.NATION_COLORS[natId];
      if (!col) return;
      // Capital = primera región asignada
  ...
  _onHover(e) {
    ...
    if (infoBar) {
      const ownerText = owner
        ? `<span style="color:${col.border}">${col.icon} ${owner==='player'?'Tu reino':owner.replace('_','')}</span>`
        : '<span style="color:#888">Territorio neutral</span>';
      const warText = warHere ? ' · <span style="color:#e05050">⚔️ EN GUERRA</span>' : '';
      const resText = Object.entries(region.baseResources)
        .filter(([,v])=>v>0)
        .map(([k,v])=>`${this._resIcon(k)}${v}`)
        .join(' ');

      infoBar.innerHTML = `
        <b style="color:#e8d090">${region.resourceIcon} ${region.name}</b>
        &nbsp;·&nbsp;${ownerText}${warText}
        &nbsp;·&nbsp;<span style="color:#c8a84b">${resText}</span>
      `;
    }
  },
  ...
  _clearHover() {
    const infoBar = document.getElementById('althoria-hover-text');
    if (infoBar) infoBar.textContent = 'Pasa el cursor sobre una región para ver información';
  },
  ...
  sync(gameState) {
    ...
    this.updateWar(gameState);
    this._buildLegend(gameState);
    // Actualizar puntos de influencia con espías activos
    this._syncSpies(gameState);
    // No re-asignar zonas (permanecen de la partida)
  },
