...
const WarSystem = {
  ...
  startWar(state, nation, targetRegionId) {
    ...
    const zones = (typeof AlthoriаMap !== 'undefined')
      ? (AlthoriаMap.nationZones[nation.id] || [])
      : [];
  ...
  _pickTargetRegion(nation) {
    if (typeof AlthoriаMap === 'undefined') return 'frontera';
    const zones = AlthoriаMap.nationZones[nation.id] || [];
...
const TerritorySystem = {
  ...
  conquer(state, nation, regionId) {
    if (typeof AlthoriаMap === 'undefined') return null;

    const natId = nation.id;
    const zones = AlthoriаMap.nationZones[natId] || [];
    ...
    // Transferir zona
    AlthoriаMap.nationZones[natId] = zones.filter(z => z !== target);
    AlthoriаMap.nationZones['player'] = AlthoriаMap.nationZones['player'] || [];
    AlthoriаMap.nationZones['player'].push(target);

    // Actualizar contadores
    state.althoriaRegions = (AlthoriаMap.nationZones['player'] || []).length;
  ...
  loseRegion(state, nation) {
    if (typeof AlthoriаMap === 'undefined') return;
    const playerZones = AlthoriаMap.nationZones['player'] || [];
    if (!playerZones.length) return;

    // Pierde la región más alejada de la capital
    const lost = playerZones[playerZones.length - 1];
    AlthoriаMap.nationZones['player'] = playerZones.filter(z => z !== lost);
    AlthoriаMap.nationZones[nation.id] = AlthoriаMap.nationZones[nation.id] || [];
    AlthoriаMap.nationZones[nation.id].push(lost);

    state.althoriaRegions = Math.max(1, (AlthoriаMap.nationZones['player'] || []).length);
    AlthoriаMap.updateWar(state);
    AlthoriаMap.render();
  },
  ...
  getRegionInfo(regionId, state) {
    ...
    if (typeof AlthoriаMap !== 'undefined') {
      Object.entries(AlthoriаMap.nationZones).forEach(([natId, zones]) => {
...
const RegionSelector = {
  ...
  // Llamado desde AlthoriаMap._onHover cuando hay click
